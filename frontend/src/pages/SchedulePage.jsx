...
import {
  ...
  Typography,
  Paper,
  TextField        // <= still needed for slotProps
} from '@mui/material'
import {
...
export default function SchedulePage () {
  ...
  /* colour palette for compressed shift-bars */
  const shiftColors = [
    'rgba(33,150,243,0.5)',  'rgba(76,175,80,0.5)',
    'rgba(255,193,7,0.5)',   'rgba(244,67,54,0.5)',
    'rgba(156,39,176,0.5)',  'rgba(0,188,212,0.5)',
    'rgba(255,87,34,0.5)'
  ]

  /* ─── 1) fetch shifts whenever the week changes ───────── */
  useEffect(() => {
    api.get('/schedule', { params: { week: weekStart.format('YYYY-MM-DD') } })
      .then(res => {
        const shifts = res.data

        /* group identical start / end combos so the calendar
           shows a single bar with a numeric badge (g.count) */
        const groups = {}
        shifts.forEach(s => {
        ...
        /* build [day][hour] staff-count table */
  ...
  /* ─── 2) staffing report for a particular day ─────────── */
  useEffect(() => {
      ...
      <Box sx={{ p: 2 }}>
            ...
            <DatePicker
              ...
              slotProps={{
                textField: {
                  size: 'small',
                  sx: { mx: 1 },
                  component: TextField   // keep the same text field
                }
              }}
            />
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, gap: 2 }}>
          ...
          <DatePicker
            ...
            slotProps={{
              textField: { size: 'small', component: TextField }
            }}
          />
        </Box>
