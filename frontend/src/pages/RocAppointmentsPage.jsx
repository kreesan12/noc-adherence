...
export default function RocAppointmentsPage() {
  ...
  // Create modal (still useful if you want click flow)
  ...
  const loadTicketsUnassigned = useCallback(async () => {
    ...
    try {
      // default: unassignedOnly=true from backend, but we make it explicit
  ...
  const doCheckAssignment = useCallback(async () => {
    ...
    try {
      // includeAssigned=1 AND unassignedOnly=0 so we can see assignment info
  ...
  const loadSchedules = useCallback(async () => {
    ...
    try {
      ...
      // fetch per tech in parallel (simple + avoids backend changes)
  ...
  useEffect(() => {
    loadSchedules()
    // load unassigned alongside schedule
  ...
  async function onPickTicketForModal(t) {
    setPickedTicket(t)
    // suggestion for “best in currently selected techs”:
    try {
      setSuggested(null)
      if (!techIds.length) return
      // Run suggestions in parallel and rank
  ...
  async function onDragEnd(evt) {
    ...
    // prevent dropping assigned tickets (shouldn’t appear in unassigned panel, but safety)
    if (ticket?.assignedTo) return

    // prevent overwrite if occupied
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={onDragEnd}>
      <Box sx={{ p: 2 }}>
        ...
        {/* Create modal: click flow (still useful) */}
