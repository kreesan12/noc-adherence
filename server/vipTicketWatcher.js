...
export async function initWhatsApp () {
  ...
  try {
    ...
    console.log('[WA] Target group JID (default):', targetGroupId)
...
// Updated: allow overriding the target WhatsApp group per message
export async function sendSlaAlert (message, opts = {}) {
  if (!sock || !isReady) throw new Error('WhatsApp client not ready')

  const overrideGroupId = opts?.groupId ? normalizeGroupId(opts.groupId) : null
  const jid = overrideGroupId || targetGroupId
