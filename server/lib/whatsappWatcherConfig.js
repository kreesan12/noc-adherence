...
function buildDefaultConfig() {
  return {
    nld: {
      ...
      digestEnabled: parseBoolean(process.env.NLD_DIGEST_ENABLED, true),
      digestIntervalMinutes: Number(process.env.NLD_DIGEST_INTERVAL_MINUTES || 60),
      digestMaxItems: Number(process.env.NLD_DIGEST_MAX_ITEMS || 5),
      templates: {
...
function sanitizeConfig(input = {}, defaults = buildDefaultConfig()) {
  ...
  return {
    nld: {
      ...
      digestEnabled: parseBoolean(source.nld?.digestEnabled, defaults.nld.digestEnabled),
      digestIntervalMinutes: parseWholeNumber(source.nld?.digestIntervalMinutes, defaults.nld.digestIntervalMinutes, { min: 15, max: 24 * 60 }),
      digestMaxItems: parseWholeNumber(source.nld?.digestMaxItems, defaults.nld.digestMaxItems, { min: 1, max: 15 }),
      templates: {
