...
export default function StaffingPage() {
  ...
  /* ─── helper: generate N-week × 5-day calendar ───────────── */
  ...
  function buildSchedule(blocks, reqMap) {
    ...
    const coverMap = {};   // heads working (ex-lunch)
    const lunchMap = {};   // heads already on lunch
    ...
    for (let ci = 0; ci < cycles; ci++) {
        ...
        group.forEach(empId => {
          getWorkDates(block.startDate, weeks).forEach(dtStr => {
            ...
            if (candidates.length){
              candidates.sort((a,b)=>
                a.surplus-b.surplus || a.lunches-b.lunches || a.h-b.h
              );
      ...
      queue.unshift(queue.pop());   // rotate queue
    }
    return schedByEmp;
  }

  /* ─── memo: reqMap, coverage, deficit, maxes ─────────────── */
  ...
  /* ─── 2) assign shifts ( respects fixed cap ) -------------- */
  const assignToStaff = async () => {
    ...
    while (anyShort(plan.deficit)){
      console.log(`[exp] cap=${hi}  short? true`);
      hi*=2;
      if (hi>10000) break;
      plan=await solve(hi);
    }
    ...
    setFixedStaff(best.headCnt);   // safe: switch OFF
  };
  ...
  async function allocateToAgents(){
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box sx={{p:3}}>
        ...
        <Box sx={{display:'flex',gap:2,flexWrap:'wrap',mb:4}}>
          ...
          <DatePicker label="Forecast Start" value={startDate}
        ...
        {/* heat-maps, tables, calendar – all unchanged (omitted for brevity) */}

        {/* … include your existing Required-Agents, Coverage,
             Under/Over, Shift-Block and Calendar sections here … */}
      </Box>
